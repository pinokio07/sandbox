...
function getMenu($name, $rel = false)
{
  $query = \App\Models\Menu::query();
  if($rel == true) {
    $query->with(['parent_items' => function ($q) {
              $q->where('active', true)
                ->with(['children' => function($c){
                  $c->with(['children' => function($dc) {
                      $dc->where('active', true)
                          ->orderBy('order', 'asc');
                    }])
                    ->where('active', true)
                    ->orderBy('order', 'asc');
  ...
  $menu = \Cache::remember($name, config('app.session_lifetime'), function() use($query, $name) {
...
function activeCompany()
{
  if (request()->session()->has('brid')) {
    ...
    $branch = \Cache::remember('user-'.$id, config('app.session_lifetime'), function() use($id) {
      return \App\Models\GlbBranch::with('company')->findOrFail($id);
    });

    return $branch;
  }
  Auth::logout();
  return redirect('/')->with('error', 'Active Company Session is empty.');
}
...
function createNotification($uid, $icon, $info, $url)
{
...
function CleanText($text)
{
  ...
  $cleanToEmpty = ["'","/","<",">","?","!","`",'【','🔥','✨','❗','➤','🔝','『Elf』','☃','👯','✿','ä','§','©','¬','ª','«','¨','¦','¢','£','°','ã','â','¥','¡','ã','¤','ï','¸','ð','¹','ë','ì','·','~','°'];
  $cleanToSpace = ["+","√","�",'】','😘','✅','☠','☀','🍀','ღ','★','🚚','💕','❤️','%','@',';','=','♕','▷','◁','®','¯','°'];

  $text = \Str::replace($cleanToEmpty, '', \Str::replace($cleanToSpace, ' ', $text));

  $text = trim(preg_replace('/\s\s+/', ' ', $text));
  $text = trim(preg_replace('/\s+/', ' ', $text));
  $text = trim(preg_replace("/\p{Han}+/u", ' ', $text));
  $text = trim(preg_replace("/\p{Han}+/u", ' ', $text));
  $text = trim(preg_replace("'/[^,;a-zA-Z0-9_-]|[,;]$/s'", ' ', $text));
