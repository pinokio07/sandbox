...
class Running
{
  ...
  public function setCode($module, $name, $date)
  {
    ...
    if($header->wasRecentlyCreated === false){ 
      if($reset == 'daily'){
        ...
        if($terakhir && $terakhir->date == '' && $terakhir->month == ''){
  ...
  public function getCodeBank($module, $name, $date)
  {

    $tanggal = \Carbon\Carbon::parse($date);
    $month = $tanggal->format('m');
    $year = $tanggal->format('Y');
    $header = \App\Models\RunningCodeHeader::where('module', $module)
                          ->where('name', $name)
                          ->first();
    if($header){
      $reset = $header->reset;
      $spec = $header->specific;
      $query = \App\Models\RunningCodeDetail::rightJoin('running_code_headers as h',
                              'running_code_details.header_id', '=', 'h.id');
      if($reset == 'month'){
        $query->where('year', $year)
              ->where('month', $month);
      ...
      if($running){
        $seq = explode('%', $running->pattern);
        $last = ($running->sequence < 1) ? 1 : $running->sequence;
        $zero = $running->leading_zero;
        $run = '';
        $roman = ['Rd', 'Rm', 'Ry', 'RY'];
        $tgls = ['d', 'm', 'M', 'y', 'Y'];
  ...
  public function setCodeBank($module, $name, $date)
  {
    $tanggal = \Carbon\Carbon::parse($date);
    $month = $tanggal->format('m');
    $year = $tanggal->format('Y');

    $header = \App\Models\RunningCodeHeader::firstOrCreate([
                'module' => $module,
                'name' => $name
              ],[
                'pattern' => 'CN / '.$name."%y%N",
