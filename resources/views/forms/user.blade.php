              ...
              <div class="col-sm-10">
                <select name="branches[]" id="branches" class="select2bs4" 
                  ...
                  @forelse ($branches as $branch)
                    <option value="{{$branch->id}}" @selected($user->branches && $user->branches->contains('id', $branch->id))>
                      {{Str::upper($branch->CB_Code) .' - '. Str::upper($branch->CB_FullName.' ( '. $branch->company->GC_Name) . ' )'}}
                    </option>
                  @empty
